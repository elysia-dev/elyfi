...
describe('MoneyPool.borrow', () => {
  ...
  before(async () => {
    ...
    await elyfiContracts.underlyingAsset.connect(deployer).transfer(investor.address, utils.parseEther('1000'));
  });
  ...
  describe('when AB token is minted by CSP', async () => {
    ...
    describe('when moneypool enough reserve', async () => {
      before(async () => {
        await elyfiContracts.underlyingAsset.connect(investor).approve(elyfiContracts.moneyPool.address, utils.parseEther('1000'));
        await elyfiContracts.moneyPool
          .connect(investor)
          .invest(elyfiContracts.underlyingAsset.address, investor.address, utils.parseEther('500'))
      })
